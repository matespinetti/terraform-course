...
module "compute" {
  ...
  private_instances1 = local.private_instances1
  private_instances2 = local.private_instances2
...
module "loadbalancer" {
  ...
  domain_name = var.domain_name
  app1_instances = {
    for k, v in module.compute.private_app1_instances : k => {
      id = v.id
    }
  }
  app2_instances = {
    for k, v in module.compute.private_app2_instances : k => {
      id = v.id
    }
  }
